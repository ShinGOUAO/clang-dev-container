...
RUN apt-get update && apt-get install -y wget git cmake swig --no-install-recommends
...
RUN apt-get update && apt-get install -y clang-17 lldb-17 lld-17 libc++-17-dev libc++abi-17-dev liblldb-17-dev && rm -rf /var/lib/apt/lists/* \
